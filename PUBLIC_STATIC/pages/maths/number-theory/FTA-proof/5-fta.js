...
<p>This is part 5, where we stand on what we've built from axioms so far, and finally prove the <DiscreetLink href="https://en.wikipedia.org/wiki/Fundamental\_theorem\_of\_arithmetic">fundamental theorem of arithmetic</DiscreetLink> (technically, the generalized version). In general, the level of rigor will decrease as the parts go on, so that the reader doesn't get bored to death. But it should be obvious how to fill out everything with complete rigor. <DiscreetLink href="https://en.wikipedia.org/wiki/Glossary\_of\_mathematical\_symbols">Here</DiscreetLink> is a glossary of math symbols.</p>
...
<blockquote>
  ...
  <p>Now, becase <Latex>$e$</Latex> is minimal, <Latex>$a$</Latex> and <Latex>$b$</Latex> must both be prime or a product of primes (else they would be in the set). Hence we can write <Latex>$a = p_1 p_2\cdots p_k$</Latex> and <Latex>$b = q_1 q_2\cdots q_j$</Latex> where these are all primes<sup className="footnote-ref" id="fnref-1"><a href="#fn-1">1</a></sup>.</p>
