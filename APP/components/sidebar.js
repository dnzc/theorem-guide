            ...
            <nav className={`md:w-0 md:h-0 fixed top-0 left-0 w-full h-[calc(2.25rem+2*1rem)] py-4 z-20`}>
                ...
                <div className='flex 2xl:block justify-start space-x-4 2xl:space-x-0 2xl:space-y-2 fixed left-5 bottom-4'>
                    ...
                    <Popup buttonStyle='text-base text-elevated mr-4 bg-body'
                        buttonContents={
                            <>
                                <div className={`hidden xs:flex items-center fixed right-4 top-4 ${active ? 'md:w-[calc(300px-2rem)] md:left-[1rem] md:top-16' : 'md:hidden'} bg-black px-3 py-1.5 [@media(hover:hover)]:hover:ring-2 ring-bold rounded-md [@media(hover:hover)]:hover:bg-white [@media(hover:hover)]:hover:bg-opacity-10`}>
